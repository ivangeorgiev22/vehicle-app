...
export class ApiClient {
  ...
  async getMission(id: number): Promise<MissionWithJobs> {
    const res = await axios.get(`${this.baseUrl}/missions/${id}`);
    return res.data;
  }

  async updateMissionStatus(id: number, mission_status: string): Promise<Mission | null> {
    const res = await axios.patch(`${this.baseUrl}/missions/${id}/status`, {mission_status});
    return res.data;
  }

  async updateJobStatus(id: number, job_status: string): Promise<Job | null> {
